---
name: noah-computer-care
description: Diagnose and repair local Windows, macOS, and Linux computer problems with read-only-first investigation, explicit change approval, platform-native commands, verification, and reusable Noah playbooks. Use for slow computers, high CPU or memory, disk cleanup, network, DNS, VPN, printers, applications, updates, backups, endpoint security, SSH keys, Wi-Fi, email, CUDA, Homebrew, OpenClaw setup, or converting troubleshooting guides into reusable playbooks.
---
...
本 Skill 不启动独立模型、不调用 Noah API，也不依赖 Noah 桌面应用。始终服从宿主 Agent 的沙箱、权限和审批机制。
...
5. 遇到 Noah 专用工具名时，读取 [tool-contract.md](references/tool-contract.md) 并转换为宿主工具或平台命令。
...
- Playbook 中的 Noah 工具名表达语义，不代表宿主必须存在同名工具；按工具契约进行映射。
...
本 Skill 派生自 [Noah for Tinkerers](https://github.com/noahapp/noah-for-tinkerers) 的公开 Playbook、工作流和安全设计，基准提交为 `e5e5536bcb925358d65d8dc6812ee6c49722373b`。当前包含 37 个保留上游作者与来源元数据的 Playbook，以及 6 个由本项目维护的 Windows/Linux 扩展。分发和修改时保留根目录 `LICENSE`、上游作者信息和来源说明。
