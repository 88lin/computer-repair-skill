...
Verify the package source and version before installing. Show the package name,
requested version, registry and exact install command, then obtain user
confirmation because this changes the machine.

If the user explicitly chooses the official shell installer, do not pipe a
remote response into a shell. Download it into an isolated temporary directory,
record the URL and SHA-256, read the script and show the relevant commands:

```bash
tmp_dir="$(mktemp -d)"
script_path="$tmp_dir/openclaw-install.sh"
...
Explain that a locally computed hash proves only which bytes were downloaded;
it is not a vendor signature. Use WAIT_FOR_USER for explicit confirmation of
the reviewed local script, then run `bash "$script_path"` and verify the result.
