...
C 盘变红、卡顿得让人想砸键盘、流氓软件怎么都请不走、网页怎么也刷不开——这些时刻，你缺的不是耐心，而是一位靠谱的师傅。Computer Repair Skill 是一个可安装到 Codex、Claude Code、OpenClaw 等 Agent 的跨平台电脑诊断与维修 Skill：它给你的 Agent 装上一整套维修规程——先取证、再给计划，经你确认才动手，全程可审计、可回滚。
...
> [!NOTE]
> C 盘变红、电脑越用越卡、流氓软件删不干净、弹窗关了又来、Wi-Fi 连着却打不开网页、系统更新后应用要搬家——用一句大白话告诉 Agent，它会先取证、再给出计划，经你确认后才动手。全程只读优先、可审计、可回滚。
...
```text
Windows 11 的 C 盘变红了，先只读盘点哪几类东西占得最多，再按风险分级给清理计划，不要直接删除任何文件。
```

```text
这台电脑越用越卡，先找出最占 CPU、内存和磁盘的进程，把原因列给我，修复前不要动任何东西。
```

```text
卸载了软件还是有残留，有的还会自启、弹广告。先只读列出残留证据，逐项确认后再清理，并告诉我怎么恢复。
```

```text
C 盘满了，想把装好的应用搬到 D 盘。先检查空间、进程占用和回滚方案，复制校验后再保留原路径。
```
