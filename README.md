...
- **按需加载**：70 个专项 Playbook 通过路由索引按问题加载，减少无关上下文，也方便维护和扩展。
...
| 能力 | 你可以这样问 |
|---|---|
| 存储健康与 SMART — 三平台读取 SMART/NVMe 计数器，判断磁盘是否将坏 | “这块硬盘是不是快坏了？先只读看 SMART 和 NVMe 计数器。” |
| Windows 崩溃分诊 — 蓝屏、意外重启、应用崩溃的转储与事件证据 | “最近老蓝屏，先列出转储文件和相关事件，不要读转储正文。” |
| 内存诊断 — 区分真实内存故障、内存压力和驱动问题 | “报内存错误，先判断是不是真的内存条坏了再谈换件。” |
| 温度与电池健康 — 过热、风扇噪音、降频、电池损耗和睡眠掉电 | “笔记本很烫还降频，先看温度、功耗和电池损耗。” |
...
| 能力 | 你可以这样问 |
|---|---|
| 蓝牙与无线外设 — 配对失败、掉连、音频卡顿和键鼠无响应 | “蓝牙耳机老掉连，先排查配对记录、电量和干扰。” |
| 显示与显卡 — 黑屏、闪屏、花屏、分辨率、外接显示器和驱动 | “外接显示器不亮，先按线材、驱动、系统三层判断。” |
...
| 能力 | 你可以这样问 |
|---|---|
| macOS 持久化审计 — LaunchAgents/Daemons、登录项、配置描述文件和扩展 | “Mac 上有个东西一直自动启动，先只读列出来源和签名。” |
| Linux 持久化审计 — systemd 单元、定时器、cron、autostart 和包归属 | “这台 Linux 有个服务停不掉，先查单元、定时器和包归属。” |
| macOS FileVault 分诊 — 加密状态、恢复密钥托管和送修前检查 | “Mac 要送修，先确认 FileVault 状态；不要显示恢复密钥。” |
| Linux 启动失败分诊 — 引导链、initramfs、根分区挂载和启动单元 | “Linux 卡在 initramfs，先定位失败阶段再动引导。” |
...
| 能力 | 你可以这样问 |
|---|---|
| 恶意软件分诊 — 先取证再清理，含凭据暴露与远程支援诈骗判断 | “怀疑中毒了，先只读取证列出可疑项，不要急着删。” |
| 勒索软件第一响应 — 隔离、保留证据、保护备份，再谈恢复 | “文件全被加密了，现在第一步该做什么？” |
...
├── references/              # 平台工具映射与 70 个专项 Playbook
