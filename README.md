...
```text
这台电脑越用越卡，开机也很慢。先找出最占 CPU、内存和磁盘的进程，列出可能原因；给出修复方案前不要做任何改动。
```

```text
有些软件卸载后仍有残留，还会自启动或弹出广告。先只读列出残留证据，逐项经我确认后再清理，并说明每项的恢复方式。
```
...
```text
Windows 只有部分网站打不开，先区分是 DNS、代理还是路由问题；未经确认，不要重置网络。
```

```text
我准备进入 PE，请先在当前 Windows 检查 BitLocker 状态；不要输出恢复密钥。
```
